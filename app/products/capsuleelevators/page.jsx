import "../../../app/products/mr-geared-elevator/mrGeared.css";

export default function MRGearedElevator() {
  return (
    <section className="mrpd2-section">
      <div className="mrpd2-container">
        {/* HERO */}
        <div className="mrpd2-hero">
          {/* LEFT — IMAGES */}
          <div className="mrpd3-media">
            <img
              src="/assets/images/products/img276.jpg"
              alt="Capsule Elevator Exterior"
            />
            <img
              src="/assets/images/products/img103.jpg"
              alt="Capsule Elevator Glass Cabin"
            />
          </div>

          {/* RIGHT — INTRO CARD */}
          <div className="mrpd2-intro">
            <h1>Capsule Elevator</h1>
            <p className="mrpd2-tag">UNOSAFE Architectural Lift Series</p>

            <p className="mrpd2-lead">
              UNOSAFE Capsule Elevator serve as architectural highlights in
              prestigious buildings. They act as elegant visual centrepieces
              that enhance structural beauty while delivering smooth and
              comfortable vertical transportation.
            </p>

            <div className="mrpd2-badges">
              <span>Architectural Landmark Design</span>
              <span>Panoramic Glass Viewing</span>
              <span>Premium Travel Comfort</span>
            </div>
          </div>
        </div>

        {/* CONTENT GRID */}
        <div className="mrpd2-grid">
          {/* DETAILS */}
          <div className="mrpd2-content">
            <h2>System Overview</h2>

            <p>
              <strong>UNOSAFE Capsule Elevator</strong> are designed to be the
              ornaments of modern buildings, adding life and elegance to their
              architecture while offering dependable vertical mobility.
            </p>

            <div className="mrpd2-feature">
              <h3>Architectural Elegance</h3>
              <p>
                Capsule Elevator enhance building aesthetics with iconic
                exterior visibility and premium structural integration, becoming
                signature visual elements of prestigious projects.
              </p>
            </div>

            <div className="mrpd2-feature">
              <h3>Design & Customization</h3>
              <p>
                Advanced design features and limitless customization options
                allow capsule Elevator to be tailored to unique architectural
                visions and creative concepts.
              </p>
            </div>

            <div className="mrpd2-feature">
              <h3>Interior Experience</h3>
              <p>
                Aesthetically attractive interiors paired with large panoramic
                glass viewing panels provide a luxurious passenger experience
                and immersive visual travel.
              </p>
            </div>

            <div className="mrpd2-feature">
              <h3>Creative Engineering</h3>
              <p>
                UNOSAFE capsule lifts are inspired by creative engineering
                excellence, crafted with artistic precision to match your
                building’s identity and elevate its presence.
              </p>
            </div>
          </div>

          {/* SPEC TABLE (UNCHANGED) */}
          <div className="mrpd2-specs">
            <h2>Technical Specifications</h2>

            <div className="mrpd2-table-wrap">
              <table className="mrpd2-table">
                <thead>
                  <tr>
                    <th colSpan="8">Panoramic Capsule Passenger Elevator</th>
                  </tr>
                  <tr>
                    <th>Capacity</th>
                    <th>Persons</th>
                    <th colSpan="2">Shaft Size (mm)</th>
                    <th colSpan="2">Car Size (mm)</th>
                    <th colSpan="2">Entrance (mm)</th>
                  </tr>
                  <tr>
                    <th></th>
                    <th></th>
                    <th>W</th>
                    <th>D</th>
                    <th>W</th>
                    <th>D</th>
                    <th>W</th>
                    <th>H</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td>6</td>
                    <td>408 kg</td>
                    <td>1600</td>
                    <td>1600</td>
                    <td>1100</td>
                    <td>1000</td>
                    <td>800</td>
                    <td>2100</td>
                  </tr>
                  <tr>
                    <td>8</td>
                    <td>544 kg</td>
                    <td>1800</td>
                    <td>1800</td>
                    <td>1300</td>
                    <td>1100</td>
                    <td>900</td>
                    <td>2100</td>
                  </tr>
                  <tr>
                    <td>10</td>
                    <td>680 kg</td>
                    <td>2000</td>
                    <td>1900</td>
                    <td>1400</td>
                    <td>1200</td>
                    <td>1000</td>
                    <td>2100</td>
                  </tr>
                  <tr>
                    <td>13</td>
                    <td>884 kg</td>
                    <td>2200</td>
                    <td>2100</td>
                    <td>1600</td>
                    <td>1400</td>
                    <td>1100</td>
                    <td>2100</td>
                  </tr>
                  <tr>
                    <td>16</td>
                    <td>1088 kg</td>
                    <td>2400</td>
                    <td>2200</td>
                    <td>1800</td>
                    <td>1500</td>
                    <td>1200</td>
                    <td>2100</td>
                  </tr>
                </tbody>
              </table>
            </div>

            <div className="mrpd2-table-wrap" style={{ marginTop: "30px" }}>
              <table className="mrpd2-table">
                <thead>
                  <tr>
                    <th colSpan="8">
                      Panoramic Capsule Elevator — Premium Series
                    </th>
                  </tr>
                  <tr>
                    <th>Capacity</th>
                    <th>Persons</th>
                    <th colSpan="2">Shaft Size (mm)</th>
                    <th colSpan="2">Car Size (mm)</th>
                    <th colSpan="2">Entrance (mm)</th>
                  </tr>
                  <tr>
                    <th></th>
                    <th></th>
                    <th>W</th>
                    <th>D</th>
                    <th>W</th>
                    <th>D</th>
                    <th>W</th>
                    <th>H</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td>8</td>
                    <td>544 kg</td>
                    <td>1900</td>
                    <td>1800</td>
                    <td>1350</td>
                    <td>1200</td>
                    <td>900</td>
                    <td>2100</td>
                  </tr>
                  <tr>
                    <td>10</td>
                    <td>680 kg</td>
                    <td>2100</td>
                    <td>2000</td>
                    <td>1500</td>
                    <td>1300</td>
                    <td>1000</td>
                    <td>2100</td>
                  </tr>
                  <tr>
                    <td>13</td>
                    <td>884 kg</td>
                    <td>2300</td>
                    <td>2200</td>
                    <td>1700</td>
                    <td>1500</td>
                    <td>1100</td>
                    <td>2100</td>
                  </tr>
                  <tr>
                    <td>16</td>
                    <td>1088 kg</td>
                    <td>2600</td>
                    <td>2400</td>
                    <td>1900</td>
                    <td>1600</td>
                    <td>1200</td>
                    <td>2100</td>
                  </tr>
                </tbody>
              </table>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
