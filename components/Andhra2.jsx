"use client";

import React from "react";
import "./bangaloreHomeElevator.css";
import Link from "next/link";

export default function Andhra2() {
  return (
    <section className="bhe-section">
      <div className="bhe-container">
        {/* LEFT CONTENT */}
        <div className="bhe-left">
          {/* IMAGE */}

          <div className="bhe-imageCard">
            <img
              src="/assets/images/products/lift-companies-in-andra-pradesh.jpg"
              alt="Lift Companies In Andhra Pradesh"
              className="bhe-image"
            />
          </div>

          {/* CONTENT */}

          <div className="bhe-content">
            <h2 className="bhe-title">Lift Companies In Andhra Pradesh</h2>

            <p className="bhe-subtitle">
              Advanced Elevator Solutions for Andhra Pradesh’s Growing
              Infrastructure
            </p>

            <p className="bhe-text">
              As Andhra Pradesh continues to grow as a hub for commerce,
              industry, and modern living, the demand for safe, efficient, and
              reliable vertical transportation is higher than ever. From
              residential apartments to commercial complexes and industrial
              warehouses, Elevator have become an essential component for
              seamless mobility.
            </p>

            <p className="bhe-text">
              <strong>UNOSAFE ELEVATOR s</strong>, a trusted name in the
              elevator industry, provides advanced lift solutions that combine
              modern technology, superior safety, and dependable service support
              across Andhra Pradesh.
            </p>

            {/* WHY BEST */}

            <h3 className="bhe-heading">Our Comprehensive Lift Solutions</h3>

            <ul className="bhe-list">
              <li>
                Passenger Lifts – Smooth and reliable Elevator ideal for
                apartments, offices, and shopping malls.
              </li>
              <li>
                Goods Lifts – Heavy-duty lifts designed for factories,
                warehouses, and industrial applications.
              </li>
              <li>
                Hydraulic Lifts – Quiet, smooth, and cost-effective lifts
                suitable for low to mid-rise buildings.
              </li>
              <li>
                Home Elevator – Stylish and compact lifts designed for villas
                and modern residential buildings.
              </li>
              <li>
                Hospital Elevator – Specially designed lifts for stretchers,
                medical equipment, and patient transport.
              </li>
            </ul>

            {/* PRICING */}

            <h3 className="bhe-heading">Features That Set Us Apart</h3>

            <p className="bhe-text">
              When comparing lift companies in Andhra Pradesh, Unosafe stands
              out with advanced engineering, modern aesthetics, and reliable
              elevator safety systems designed for long-term performance.
            </p>

            <div className="bhe-priceCard">
              <h4>Advanced Safety Features</h4>
              <p>Overload protection systems</p>
              <p>Automatic Rescue Device (ARD)</p>
              <p>Emergency alarms and intercom systems</p>

              <h4>Modern Aesthetic Designs</h4>
              <p>Mild-steel elevator cabins</p>
              <p>Luxury stainless steel interiors</p>
              <p>Premium glass elevator cabin options</p>

              <h4>Energy Efficient Technology</h4>
              <p>Machine Room Less (MRL) lift systems</p>
              <p>Reduced power consumption</p>
              <p>Optimized performance with modern drive technology</p>
            </div>

            {/* VARIANTS */}

            <h3 className="bhe-heading">End-to-End Services We Offer</h3>

            <ul className="bhe-list">
              <li>
                Design & Consultation – Experts evaluate building layouts and
                recommend the best elevator type and capacity.
              </li>
              <li>
                Manufacturing – Premium-grade materials and advanced technology
                ensure durable elevator systems.
              </li>
              <li>
                Installation – Professional teams ensure safe and precise
                installation.
              </li>
              <li>
                Modernization – Upgrade older Elevator with modern safety
                features and improved aesthetics.
              </li>
              <li>
                Annual Maintenance Contracts (AMC) – Comprehensive maintenance
                services for long-term reliability.
              </li>
            </ul>

            {/* BENEFITS */}

            <h3 className="bhe-heading">Our Safety Systems</h3>

            <ul className="bhe-list">
              <li>Safety Management System for regulatory compliance</li>
              <li>Express Support System for rapid issue resolution</li>
              <li>SMS Notification System for real-time service updates</li>
              <li>Regular safety inspections and performance checks</li>
              <li>Continuous customer feedback improvement system</li>
            </ul>

            {/* WHY CHOOSE US */}

            <h3 className="bhe-heading">
              Why Unosafe is the Best Lift Company in Andhra Pradesh
            </h3>

            <p className="bhe-text">
              With years of industry experience and a strong commitment to
              quality, UNOSAFE ELEVATOR s has become a trusted partner for
              architects, builders, and developers across Andhra Pradesh. Our
              Elevator are designed to deliver smooth performance, durability,
              and complete safety.
            </p>

            <p className="bhe-text">
              Our customer-first approach combined with innovative technology
              and reliable service support ensures every project receives the
              highest standard of vertical transportation solutions.
            </p>

            {/* CONCLUSION */}

            <h3 className="bhe-heading">Conclusion</h3>

            <p className="bhe-text">
              If you are searching for reliable lift companies in Andhra
              Pradesh, UNOSAFE ELEVATOR s offers advanced elevator systems
              backed by expert installation and long-term service support. Our
              solutions ensure safe, efficient, and dependable vertical mobility
              for residential, commercial, and industrial buildings.
            </p>
          </div>
        </div>
        {/* RIGHT SIDEBAR */}

        <div className="bhe-sidebar">
          <div className="bhe-card">
            <h3 className="bhe-cardTitle">Bangalore</h3>

            <Link href="/Bangalore" className="bhe-item">
              Home Elevator in Bangalore
              <span className="bhe-check">✓</span>
            </Link>
          </div>

          <div className="bhe-card">
            <h3 className="bhe-cardTitle">Chennai</h3>

            <Link href="/Chennai1" className="bhe-item">
              Goods Lift Manufacturers In Chennai{" "}
              <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Chennai2" className="bhe-item">
              Lift Companies In Chennai <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Chennai3" className="bhe-item">
              KONE Elevator In Chennai <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Chennai4" className="bhe-item">
              Elite Elevator Chennai <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Chennai5" className="bhe-item">
              Hydraulic Lift Manufacturers In Chennai{" "}
              <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Chennai6" className="bhe-item">
              Home Elevator In Chennai <span className="bhe-check">✓</span>
            </Link>
          </div>

          <div className="bhe-card">
            <h3 className="bhe-cardTitle">Kerala</h3>

            <Link href="/Kerala1" className="bhe-item">
              Goods Lift Manufacturers In Kerala{" "}
              <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Kerala2" className="bhe-item">
              Lift Companies In Kerala <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Kerala3" className="bhe-item">
              Kone Elevator In Kerala <span className="bhe-check">✓</span>
            </Link>

            <Link href="/Kerala5" className="bhe-item">
              Elite Elevator in Kerala<span className="bhe-check">✓</span>
            </Link>
            <Link href="/Kerala6" className="bhe-item">
              Hydraulic Lift Manufactures In kerala{" "}
              <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Kerala7" className="bhe-item">
              Best Elevator Manufactureres in kerala{" "}
              <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Kerala8" className="bhe-item">
              Home Elevator in kerala <span className="bhe-check">✓</span>
            </Link>
          </div>

          <div className="bhe-card">
            <h3 className="bhe-cardTitle">Andhra Pradesh</h3>

            <Link href="/Andhra1" className="bhe-item">
              Goods Lift Manufacturers In Andhra Pradesh{" "}
              <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Andhra2" className="bhe-item">
              Lift Companies In Andhra Pradesh{" "}
              <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Andhra3" className="bhe-item">
              Kone Elevator In Andhra Pradesh{" "}
              <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Andhra4" className="bhe-item">
              Elite Elevator In Andhra Pradesh{" "}
              <span className="bhe-check">✓</span>
            </Link>

            <Link href="/Andhra5" className="bhe-item">
              Hydraulice Lift Manufactureres In Andhra Pradesh{" "}
              <span className="bhe-check">✓</span>
            </Link>
            <Link href="/Andhra6" className="bhe-item">
              Capsule Elevator in Hyderabad <span className="bhe-check">✓</span>
            </Link>

            <Link href="/Andhra7" className="bhe-item">
              Esco Lifts India-Anantpur-UNOSAFE{" "}
              <span className="bhe-check">✓</span>
            </Link>

            <Link href="/Andhra8" className="bhe-item">
              Home Elevator in Andhra Pradesh{" "}
              <span className="bhe-check">✓</span>
            </Link>
          </div>
        </div>
      </div>
    </section>
  );
}
